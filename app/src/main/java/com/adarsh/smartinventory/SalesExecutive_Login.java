...
EditText empcd,password;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_sales_executive__login);
        empcd=findViewById(R.id.stafflogin_empcd);
        password=findViewById(R.id.stafflogin_pswd);
    }

    public void staffLogin(View view) {
        Api api= Api_client.SmartInventory().create(Api.class);
        api.STAFF_LOGIN_CALL(empcd.getText().toString(),password.getText().toString()).enqueue(new Callback<StaffLogin>() {
            @Override
